...
2. knowledge_fetcher: TB_LOAN_BAL 테이블 발견, 잔액 컬럼 확인
...
**파이프라인 경과:**
1. knowledge_fetcher: `search_table_meta("연체율")` → 테이블 메타에 "연체율" 컬럼 없음
2. knowledge_interpreter: `measure:연체율` UNRESOLVED 유지
...
6. knowledge_interpreter: `measure:연체율` → CONFIRMED, value="연체금액합/대출잔액합×100"
...
**파이프라인 경과:**
1. knowledge_fetcher: `search_code_meta("CUS_GRD_CD")`
...
- recovery_agent가 이 모호성을 감지해도 **사용자에게 명확화 질문을 트리거할 경로가 없음** (recovery → knowledge_fetcher 루프만 가능)
...
**파이프라인 경과:**
1. knowledge_fetcher: "계좌잔액"으로 검색 → TB_ACCOUNT_BAL 하나만 발견
...
현재 루프에서는 `recovery → knowledge_fetcher` 경로만 존재한다.
