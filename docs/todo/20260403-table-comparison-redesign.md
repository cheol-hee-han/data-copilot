...
- context_interpreter가 배치 해석에서 selected/rejected를 동시 처리하고 있음
- 정보계 DB에 같은 도메인의 유사 테이블이 다수 존재하는 것이 프로젝트 핵심 도전
- state-architecture.md에서도 갭으로 인식: "비교 결과가 knowledge에 구조적으로 저장되지 않음"

## 현재 문제

1. context_interpreter가 도구 결과 해석 + knowledge 승격 + 테이블 판정 + use_case 평가를 동시 수행 → 판단 부담 과중
...
- 첫 번째 해석(context_interpreter)에서 선택한 테이블로 SQL 생성이 실패한 경우, 처음에 rejected된 테이블이 실제로는 더 적합할 수 있음
...
- context_interpreter에 조건부 분기 추가보다 단순
...
4. **출력 구조**: context_interpreter와 동일하게 `[{"table_name": "...", "reason": "..."}]`
...
```
1차: context_interpreter → TB_A(selected), TB_B(rejected), TB_C(rejected)
...
### 대안: context_interpreter 내부 조건부 호출

- selected 중 같은 주제영역 테이블이 2개 이상이면 table_comparison LLM 자동 호출
- 장점: recovery 루프 없이 1차 해석에서 해소
- 단점: context_interpreter의 복잡도 증가, 실패 사유 없이 비교하므로 판정 근거가 약함

## 우선순위

중 — 현재 context_interpreter의 배치 판정으로도 동작은 하지만, 유사 테이블이 많은 실제 은행 데이터에서 정확도 개선 필요

## 관련 파일

- `resources/prompts/reason/table_comparison_system.txt` — 현재 미사용 프롬프트
- `src/agents/nodes/reason/context_interpreter.py` — TABLE_COMPARISON_SYSTEM import만 존재
