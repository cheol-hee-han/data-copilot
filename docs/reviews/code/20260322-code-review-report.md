...
- **검토 일시**: 2026-03-22
- **검토 대상**: `src/` 디렉토리 전체, 프로젝트 디렉토리 구조, `tests/`, `devtools/`
...
- **위치**: `src/agents/nodes/interpret/query_normalizer.py:26-29`
- **위반 규칙**: 코드 품질
...
### W-11. `devtools/scripts/` — 실행 맥락 혼재

- **위치**: `devtools/scripts/` (10개 파일: .py + .sh + .sql 혼재)
...
- **위치**: `src/agents/nodes/interpret/preprocessor.py:74`, `src/main.py:153-174`
...
- **위치**: `src/utils/security.py`, `src/agents/nodes/interpret/preprocessor.py`
...
- **위치**: `devtools/scripts/init_postgres.sql`, `devtools/docker/scripts/init_postgres.sql`
- **문제**: 두 파일의 내용이 동일하다. 한쪽을 수정하면 다른쪽도 수정해야 하므로 동기화 누락 위험이 있다.
- **개선**: `devtools/docker/scripts/init_postgres.sql`을 정본으로 유지하고 다른쪽은 삭제.
