...
`standalone/docker/elasticsearch/Dockerfile`에서 `analysis-nori` 플러그인을 포함한 커스텀 이미지를 빌드한다.
...
| `src/agents/nodes/preprocessor.py` | SQL 인젝션 패턴 13종, 프롬프트 인젝션 통합, 유니코드 정규화, 로그 PII 마스킹 |
