...
def test_chain_validator_feedback_to_generator():
    """검증 실패 시 validation_feedback 이 재생성 입력에 반영된다."""
    ...
    state = PipelineState(
        ...
        sql_retry_count=0,
        preprocessed_input="고객 이름 변경",
        context=ContextInfo(),
    )
    val_result = validate_sql_node(state)

    assert len(val_result.get("sql_validation_errors", [])) > 0
    assert len(val_result.get("validation_feedback", "")) > 0

    # Step 2: 피드백이 다음 state 에 주입 가능한지 확인
    state_for_retry = state.model_copy(update=val_result)
    assert state_for_retry.validation_feedback != ""
    assert "UPDATE" in state_for_retry.validation_feedback

    log_test_case(logger, "test_chain_validator_to_generator",
                  "UPDATE TB_CUST",
                  "validation_feedback 비어있지 않음",
                  state_for_retry.validation_feedback[:80],
                  True)
...
def test_chain_validator_pass_to_executor_state():
    """검증 통과된 SQL 이 executor 입력 state 에 올바르게 설정된다."""
    from src.agents.nodes.sql_validator import validate_sql_node
    ...
    state = PipelineState(
        ...
        context=ContextInfo(
            table_metas=[TableMeta(table_name="TB_CUST_INFO", table_description="고객")]
        ),
    )
    val_result = validate_sql_node(state)

    # 검증 통과 시 validated_sql 이 설정됨
    assert val_result["status"] == QueryStatus.SQL_VALIDATED
    assert val_result["validated_sql"] == valid_sql

    # executor 에 전달할 state 구성
    state_for_exec = state.model_copy(update=val_result)
    assert state_for_exec.validated_sql == valid_sql
    assert state_for_exec.sql_validation_errors == []

    log_test_case(logger, "test_chain_validator_to_executor",
                  valid_sql[:40],
                  "validated_sql 설정됨",
                  state_for_exec.validated_sql[:40],
                  True)
...
async def test_chain_clarification_roundtrip():
    ...
    with patch(
        "src.agents.nodes.history_resolver.resolve_history",
