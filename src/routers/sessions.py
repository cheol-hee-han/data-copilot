...
@router.get("/turns/{turn_id}/metadata", response_model=TurnMetadataResponse)
async def get_turn_metadata(turn_id: str):
    """특정 턴의 metadata를 조회한다 (Tier 2)."""
    result = await session_service.get_turn_metadata(_pool(), turn_id)
    if result is None:
        raise HTTPException(404, "턴을 찾을 수 없습니다.")
    return result


@router.patch("/turns/{turn_id}/like", response_model=LikeResponse)
async def toggle_like(turn_id: str, body: LikeRequest):
    """턴에 좋아요/싫어요를 설정한다."""
    result = await session_service.toggle_like(
        _pool(), turn_id, body.is_liked, body.feedback,
    )
    if result is None:
        raise HTTPException(404, "턴을 찾을 수 없습니다.")
    return result
...
@router.patch("/turns/{turn_id}/download", response_model=DownloadResponse)
async def mark_download(turn_id: str):
    """턴의 다운로드를 기록한다."""
    result = await session_service.mark_downloaded(_pool(), turn_id)
    if result is None:
        raise HTTPException(404, "턴을 찾을 수 없습니다.")
    return result
