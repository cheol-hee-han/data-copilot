"""BDP 커넥터 — Cloudera CDP 7.1.9 Impala 기반 BDP 업무 시스템 읽기 전용 쿼리 실행.
...
핵심 함수/클래스:
    - BDPConnector: DatabaseConnector 구현체, 읽기 전용 쿼리 실행
...
드라이버 설정 필드(settings.impala_*)는 드라이버/물리 축이므로 그대로 유지한다.
"""
...
class BDPConnector(DatabaseConnector):
    """BDP 업무 시스템 커넥터 — Cloudera CDP 7.1.9 Impala 드라이버 기반 (읽기 전용).
    ...
    async def connect(self) -> None:
        """Impala 연결을 초기화한다."""
        if self._use_dummy:
            logger.info("BDP(Impala) Dummy 모드로 초기화")
            return
    ...
    async def execute_query(
        ...
        if self._use_dummy:
            logger.info("BDP(Impala) Dummy 쿼리 실행", sql=query)
