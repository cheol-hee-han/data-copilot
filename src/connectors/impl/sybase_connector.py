...
핵심 함수/클래스:
    - SybaseIQConnector: DatabaseConnector 구현체, 읽기 전용 쿼리 실행
...
class SybaseIQConnector(DatabaseConnector):
    """SAP Sybase IQ 16.1 커넥터 (읽기 전용).
    ...
    async def execute_query(
        ...
        if self._use_dummy:
            logger.info(
                "Sybase IQ Dummy 쿼리 실행", sql=query,
            )
