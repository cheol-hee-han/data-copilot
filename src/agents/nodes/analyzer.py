...
핵심 함수:
    - analyze_data_node: state.user_input, state.sql_result 을 읽어
...
위임 구조:
    ...
    - 프롬프트: nodes/prompts/system_prompts.py 에서 DATA_ANALYSIS, ANALYSIS_USER,
      VISUALIZATION_JUDGMENT, VISUALIZATION_SVG_GENERATION 등을 로드하여 서비스에 주입
...
from src.agents.nodes.prompts.system_prompts import (
    ANALYSIS_FORMAT_HINT,
    ANALYSIS_USER,
    DATA_ANALYSIS,
    VISUALIZATION_JUDGMENT,
    VISUALIZATION_SVG_GENERATION,
    VIZ_JUDGMENT_FORMAT_HINT,
    VIZ_SVG_SYSTEM,
)
...
async def analyze_data_node(
    ...
    try:
        analysis, viz = await analyze_data(
            ...
            format_hint=ANALYSIS_FORMAT_HINT,
            viz_judgment_prompt=VISUALIZATION_JUDGMENT,
            viz_judgment_hint=VIZ_JUDGMENT_FORMAT_HINT,
            viz_svg_template=VISUALIZATION_SVG_GENERATION,
