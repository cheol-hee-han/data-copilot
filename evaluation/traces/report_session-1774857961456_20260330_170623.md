...
| # | 노드 | 유형 | 결정 | 확신도 | 근거 |
|--:|------|------|------|-------:|------|
| 1 | classify_intent | intent_classification | data_extraction | 95% | category=DATA_EXTRACTION, 지점별 여신 잔액이라는 구체적 수치와 상위 … |
...
| clarify | clarification_question | → `어떤 기간 기준으로 지점별 여신 잔액을 확인하시겠어요?  1) 오늘(최신…` |
|  | formatted_response | → `어떤 기간 기준으로 지점별 여신 잔액을 확인하시겠어요?  1) 오늘(최신…` |
...
sequenceDiagram
    ...
    intentgate->>+LLM: [7] LLM(gemini-3.1-flash-lite-preview) 1126tok
    LLM-->>-intentgate: 응답 2272ms
    ...
    normalizationphase1->>+LLM: [11] LLM(gemini-3.1-flash-lite-preview) 9120tok
    LLM-->>-normalizationphase1: 응답 6553ms
    normalizationphase2->>+LLM: [12] LLM(gemini-3.1-flash-lite-preview) 2414tok
    LLM-->>-normalizationphase2: 응답 4421ms
    Note over normalizequery: [13] ⚡ normalization: RANK
    deactivate normalizequery
    normalizequery->>+clarify: [15] clarify 시작
    ->>+LLM: [16] LLM(gemini-3.1-flash-lite-preview) 1511tok
    LLM-->>-: 응답 2184ms
    deactivate clarify
```
...
gantt
    ...
    section 초기 실행
    preprocess : 0, 0.1s
    resolve_history : 0, 3.2s
    classify_intent : 0, 2.3s
    normalize_query :crit, 0, 11.0s
    clarify : 0, 2.2s
```
...
| 노드 | 호출 수 | 토큰 | 소요시간 | 비중 |
|------|-------:|-----:|--------:|-----:|
| normalization_phase1 | 1 | 9,120 | 6.6s | 58% |
| normalization_phase2 | 1 | 2,414 | 4.4s | 15% |
|  | 1 | 1,511 | 2.2s | 10% |
| 이력해소 | 1 | 1,447 | 2.2s | 9% |
| intent_gate | 1 | 1,126 | 2.3s | 7% |
| **합계** | **5** | **15,618** | **17.7s** | 100% |
...
| 4 | 🤖 llm_call | 이력해소 | LLM(gemini-3.1-flash-lite-preview) 1447t… | gemini-3.1-flash-lite-preview 1407+40tok | 2225ms | - | - |
| 5 | ■ node_end | resolve_history | resolve_history 완료 | 3개 필드 변경 | 3249ms | success | awaiting_clarification: False, clarifica… |
| 6 | ▶ node_start | classify_intent | classify_intent 시작 | - | - | - | - |
| 7 | 🤖 llm_call | intent_gate | LLM(gemini-3.1-flash-lite-preview) 1126t… | gemini-3.1-flash-lite-preview 1062+64tok | 2272ms | - | - |
| 8 |   ⚡ decision | classify_intent | intent_classification: data_extraction |  (95%) category=DATA_EXTRACTION,… | - | - | - |
| 9 | ■ node_end | classify_intent | classify_intent 완료 | 4개 필드 변경 | 2276ms | success | intent: data_extraction, intent_confiden… |
| 10 | ▶ node_start | normalize_query | normalize_query 시작 | - | - | - | - |
| 11 | 🤖 llm_call | normalization_phase1 | LLM(gemini-3.1-flash-lite-preview) 9120t… | gemini-3.1-flash-lite-preview 8424+696tok | 6553ms | - | - |
| 12 | 🤖 llm_call | normalization_phase2 | LLM(gemini-3.1-flash-lite-preview) 2414t… | gemini-3.1-flash-lite-preview 1716+698tok | 4421ms | - | - |
| 13 |   ⚡ decision | normalize_query | normalization: RANK |  (0%) entities=['지점', '여신'], me… | - | - | - |
| 14 | ■ node_end | normalize_query | normalize_query 완료 | 3개 필드 변경 | 10980ms | success | normalized_query: original_query='지점별 여신… |
| 15 | ▶ node_start | clarify | clarify 시작 | - | - | - | - |
| 16 | 🤖 llm_call |  | LLM(gemini-3.1-flash-lite-preview) 1511t… | gemini-3.1-flash-lite-preview 1453+58tok | 2184ms | - | - |
| 17 | ■ node_end | clarify | clarify 완료 | 5개 필드 변경 | 2186ms | success | clarification_question: 어떤 기간 기준으로 지점별 여… |
