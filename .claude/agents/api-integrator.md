...
name: api-integrator
description: |
  MongoDB·Qdrant·PostgreSQL·Redis 등 사내 다중 데이터 소스 연동을 구현합니다.
...
1. **MongoDB** — 테이블/컬럼 레이아웃, 코드 메타, 비즈용어 사전
2. **Qdrant** — 업무 매뉴얼 벡터 검색 (RAG), 과거 SQL 이력
3. **PostgreSQL (정보계)** — 읽기 전용 데이터 추출
4. **Redis** — 캐시 (메타데이터, 빈도 높은 쿼리 결과)
