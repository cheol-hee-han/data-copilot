...
## 최상위 작업 원칙 (모든 에이전트 공통)

**전체 관점 사고(Holistic Thinking)** — 작은 수정·보완이라도 좁은 시야로 국소 대응하지 말고,
디자인·일관성·유지보수성·효율성·기능·성능 6가지 관점에서 전체 구조를 한 번 더 점검한 뒤 최적안을 도출한다.
서브에이전트 호출 시에도 이 원칙을 작업 지시에 포함한다. 상세: `.claude/rules/holistic-thinking.md`
...
**모든 문서(memory 파일·design doc·todo·report·README 등 `.md` 전체)는 본문 첫머리에 목차(Table of Contents)를 둔다.**

- 목적: 독자(사람·LLM 모두)가 목차만 읽고 필요한 섹션만 부분 로딩(`Read offset/limit`)할 수 있게 하여 **컨텍스트 소비를 최소화**한다.
- 형식: 문서 제목(H1) 바로 아래 `## 목차` 섹션을 두고, 각 H2 섹션을 `- [섹션명](#anchor) — 한 줄 요약` 형태로 나열한다. 필요 시 라인 번호 힌트(`L42-L80`)도 함께 적는다.
- 짧은 문서(단일 섹션·30줄 미만)는 목차 생략 가능하나, 그 외에는 예외 없이 둔다.
- `MEMORY.md`는 목차 그 자체이므로 추가 목차 불필요(현재 형식 유지).
- 신규 문서뿐 아니라 **기존 문서 수정 시에도** 목차가 없거나 outdated면 함께 갱신한다.

**메모리 인덱스 사용 원칙**: 메모리를 회상할 때는 먼저 `MEMORY.md` 한 줄 요약만 훑고, 실제로 필요한 항목만 개별 파일을 읽는다. 모든 메모리 파일을 일괄 로드하지 않는다.
...
- 전체 관점 사고 원칙: `.claude/rules/holistic-thinking.md`
