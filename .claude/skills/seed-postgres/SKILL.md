...
- ES `table_meta`/`column_meta`에 등록된 테이블과 PG DDL은 반드시 1:1 대응
- 에이전트가 ES 메타를 참조하여 SQL을 생성하면, 해당 SQL이 PG에서 실행 가능해야 함
...
- **TYPE-2: 코드값 불일치** — 랜덤 데이터의 약 3%에 ES 메타에 미정의 코드 삽입
- **TYPE-3**: PG에서 직접 해당 없음 (ES 메타 연계)
