...
**가장 중요한 주석.** 파일을 열었을 때 "이 파일이 뭔지, 왜 존재하는지"를 즉시 파악할 수 있어야 한다. 특히, NL to SQL에 대한 해당 모듈의 주요 아이디어가 있거나 결정적 역할을 하는 경우 상세 설명이 필수다.
가장 윗 줄 파일 "제목 - 설명"의 2줄 아래에 반드시 작성자 '한철희'와 최종수정일시를 기재. 
(작성자: 한철희 / 최종수정: yyyy-mm-dd HH:MI:SS — Bash로 `date +"%Y-%m-%d %H:%M:%S"` 실행하여 현재 시각 사용)
...
"이 클래스가 무엇을 나타내는지"를 한 줄로 요약한다. 클래스가 복잡하거나 NL to SQL에서 핵심적인 역할을 한다면, 추가로 "왜 이 클래스가 필요한지"와 "주요 속성/메서드 설명"을 포함하는 상세 설명이 필요하다. (특히, state 객체의 변경이 있는 경우 의도와 맥락 설명이 중요하다)
